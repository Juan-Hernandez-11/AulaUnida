...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!documentNumber || !/^\d{5,}$/.test(documentNumber)) {
      return NextResponse.json({ error: 'El número de documento debe ser numérico y mínimo 5 dígitos.' }, { status: 400 });
    }
    ...
    if (!name || !/^[A-Za-zÁÉÍÓÚáéíóúÑñ ]{3,}$/.test(name.trim())) {
      return NextResponse.json({ error: 'El nombre debe tener solo letras y al menos 3 caracteres.' }, { status: 400 });
    }
    ...
    if (age < 5) {
      return NextResponse.json({ error: 'La edad mínima es 5 años.' }, { status: 400 });
    }
    // Teléfono: solo números, 7-10 dígitos, no empieza por 0
    if (!phone || !/^\d{7,10}$/.test(phone) || phone.startsWith('0')) {
      return NextResponse.json({ error: 'El teléfono debe tener entre 7 y 10 dígitos, no empezar por 0.' }, { status: 400 });
    }
    // Dirección: mínimo 5 caracteres
    if (!address || address.trim().length < 5) {
      return NextResponse.json({ error: 'La dirección debe tener al menos 5 caracteres.' }, { status: 400 });
    }
    // Género: solo M, F, O
    if (!['M', 'F', 'O'].includes(gender)) {
      return NextResponse.json({ error: 'El género debe ser M, F u O.' }, { status: 400 });
    }
    // Foto: si se envía, debe ser url de imagen (opcional, solo si se usa)
    if (photoUrl && !/^https?:\/\/.+\.(jpg|jpeg|png|gif|webp)$/i.test(photoUrl)) {
      return NextResponse.json({ error: 'La foto debe ser una URL de imagen válida.' }, { status: 400 });
    }
    ...
    try {
      ...
      return NextResponse.json({ error: 'El correo ya está registrado en Firebase Auth.' }, { status: 400 });
    } catch (e) {
